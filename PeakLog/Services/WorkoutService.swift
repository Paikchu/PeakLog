...
final class MockWorkoutService: WorkoutServiceProtocol {
    private let local = LocalWorkoutService(database: .shared)

    func updateExerciseName(sessionId: String, exerciseId: String, name: String) async throws -> Exercise {
        try await local.updateExerciseName(sessionId: sessionId, exerciseId: exerciseId, name: name)
    }

    func updateSet(sessionId: String, exerciseId: String, setId: String, weight: Double?, weightUnit: WeightUnit, reps: Int) async throws -> ExerciseSet {
        try await local.updateSet(sessionId: sessionId, exerciseId: exerciseId, setId: setId, weight: weight, weightUnit: weightUnit, reps: reps)
    }

    func addSet(sessionId: String, exerciseId: String, weight: Double?, weightUnit: WeightUnit, reps: Int) async throws -> ExerciseSet {
        try await local.addSet(sessionId: sessionId, exerciseId: exerciseId, weight: weight, weightUnit: weightUnit, reps: reps)
    }

    func deleteSet(sessionId: String, exerciseId: String, setId: String) async throws {
        try await local.deleteSet(sessionId: sessionId, exerciseId: exerciseId, setId: setId)
    }

    func deleteExercise(sessionId: String, exerciseId: String) async throws {
        try await local.deleteExercise(sessionId: sessionId, exerciseId: exerciseId)
    }

    func updateSetRPE(setId: String, rpe: Double?) async throws -> ExerciseSet {
        try await local.updateSetRPE(setId: setId, rpe: rpe)
    }

    func activeDaysInMonth(year: Int, month: Int) async throws -> [Date] {
        try await local.activeDaysInMonth(year: year, month: month)
    }

    func sessionsForDay(_ date: Date) async throws -> [WorkoutSession] {
        try await local.sessionsForDay(date)
    }

    func activeRunningDaysInMonth(year: Int, month: Int) async throws -> [Date] {
        try await local.activeRunningDaysInMonth(year: year, month: month)
    }

    func runningRecordsForDay(_ date: Date) async throws -> [RunningWorkoutRecord] {
        try await local.runningRecordsForDay(date)
    }

    func createStrengthSession(_ draft: StrengthSessionDraft) async throws -> WorkoutSession {
        try await local.createStrengthSession(draft)
    }
    ...
    func createRunningRecord(
        ...
        try await local.createRunningRecord(workoutDate: workoutDate, durationMinutes: durationMinutes, distanceKm: distanceKm, source: source)
    }
}
