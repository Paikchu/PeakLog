...
- `edge-function-release-runbook.md`  
  Edge Function 发布与部署一致性 Runbook：从哪个 commit 部署、部署后如何回读校验线上字节、检测到漂移时如何判明方向并回滚。配套 `backend/scripts/verify-deployed-function.mjs` 与 `.github/workflows/backend.yml`。因 Issue #133（线上热修与 `main` 漂移，重部署会静默恢复错误的周计划生成窗口）而立。
...
5. 部署 / 回滚 Edge Function：读 `edge-function-release-runbook.md`
