...
| `workout_sessions` / `exercises` / `exercise_sets` | ALL（own） | 力量训练三层聚合，`CloudMapper.pushBundle` 统一 upsert。**删除不走「缺席对账」**：客户端只 DELETE 本地墓碑日志点名的 id（Issue #132），另一台设备则从 `record_deletions` 读回删除意图（Issue #148） |
...
| `record_deletions` | **仅 SELECT** | 服务端墓碑日志（Issue #148）：上面五张记录表每被删掉一行就写一条，含 `ON DELETE CASCADE` 带走的子行。写入方是 `AFTER DELETE` 触发器（`SECURITY DEFINER`），**没有 INSERT/UPDATE/DELETE 策略**——伪造的墓碑会在用户所有设备上删数据，可擦除的墓碑会复活已删记录。pull 按 `deleted_at` 增量读回并应用到本地缓存。保留 180 天，`purge_record_deletions()` 每日清理 |
